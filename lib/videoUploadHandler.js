...
module.exports = function (videoRoot,imageRoot,db,fileData,category,callback) {
   var picPath = imageRoot + fileData.name.substring(0,fileData.name.lastIndexOf('.'))+'.jpg';
    ...
    Transcoder.prototype.transcode = function(inputPath,outputPath,callback){
        console.log('7777777777777777777777777');
        console.log(this.transcoding)
        var task =  Array.prototype.slice.call(arguments,0);
        this.transcodeTask.push(task);
        if(!this.transcoding){
            this.doTranscode(this.transcodeTask.shift());
        }
    };
    Transcoder.prototype.doTranscode=function(params){
        var me = this;
        me.transcoding = true;
        executor.execute(params[0], params[1], function (err, ret) {
            if (ret == 'complete') {
                var status = err ? -1 : 1;
                F_SHOT().screenshot(outputPath,picPath,function(err,d) {
                    ...
                    collection.update({'_id': insertId}, {$set: {status: status,dur:d}}, function (err, ret) {
                        ...
                        //    }
                        //})
                        if(me.transcodeTask.length==0){
                            me.transcoding=false;
                        }else{
                            me.doTranscode(me.transcodeTask.shift());
                        }
